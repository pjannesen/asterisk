...
static int action_redirect(struct mansession *s, const struct message *m)
{
	...
	if (ast_strlen_zero(name2)) {
		...
		res = ast_async_goto_and_discard_after_goto(chan, context, exten, pi);
		if (!res) {
	...
	if (!res) {
		if (!ast_strlen_zero(context2)) {
			res = ast_async_goto_and_discard_after_goto(chan2, context2, exten2, pi2);
		} else {
			res = ast_async_goto_and_discard_after_goto(chan2, context, exten, pi);
		}
		if (!res) {
